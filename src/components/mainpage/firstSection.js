...
const FirstSection = ({ ...props }) => {
  ...
  const bgImage = convertToBgImage(
    getImage(
      FirstSectionQuery.wp.generalSettings.mainSettings.backgroundFirstScreen
        .localFile
    )
  )
  ...
  return (
    ...
    >
      <Container
        as="div"
        sx={{ alignItems: "center", height: "calc(100vh - 120px)" }}
      >
        <Box sx={{ maxWidth: "700px", pb: [4, 5, 6] }}>
          <Heading variant="subTitle" as="h1" sx={{ mb: [2, 3, 4] }}>
            Рекламное агентство УфаПечать
          </Heading>
          <RunningText mb={[2, 3, 4]} height={"60px"} words={titles}>
            ВСЕ ВИДЫ
          </RunningText>
